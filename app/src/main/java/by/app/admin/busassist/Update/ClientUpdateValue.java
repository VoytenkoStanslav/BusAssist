...
public class ClientUpdateValue extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            receivedClientId = getIntent().getLongExtra("CLIENT_ID", 1);
            Log.e("LooogUp", String.valueOf(receivedClientId));
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        int zacaz = queriedClient.getType_pos_client();
        path = queriedClient.getImage_path_client();
        try{
            String path = queriedClient.getImage_path_client();
        ...
        spinnerClientType.setSelection(zacaz);
        if (zacaz == 2) {
            tvCompany.setVisibility(View.VISIBLE);
            editCompanyClient.setVisibility(View.VISIBLE);
            editCompanyClient.setText(queriedClient.getCompany_client());
        }
        editNameClient.setText(String.valueOf(queriedClient.getName_client()));
        String number = queriedClient.getNumber_client();
        editNumberClient.setText(number);
        editSkypeClient.setText(queriedClient.getSkype_client());
        editProjectClient.setText(queriedClient.getProject_client());
        editCommitClient.setText(queriedClient.getCommit_client());
            ...
            if (!name_client.isEmpty()) {
                ...
                if (!number_client.isEmpty()) {
                    ...
                    if (!project_client.isEmpty()) {
                        ...
                        Client updateClient = new Client(image_path_client, type_client, type_pos_client,
                                company_client, name_client, number_client, skype_client, commit_client,
                                project_client);
                        dbHelper.updateClient(receivedClientId, this, updateClient);
                        goBackHome();
                    } else {
